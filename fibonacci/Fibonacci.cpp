...
vector<vector<double>> IdentityM(vector<vector<double>> v){ //returns the identitymatrix with the dimensions of v
	assert(v.size()==v[0].size());                          //v.size=number of rows, v[0].size=number of columns in v[0] <=> number of cloumns in v
	unsigned int i;
	unsigned int j;
	for (i=0; i<v.size(); i++){
		for (j=0; j<v.size();j++){
...
vector<vector<double> > MatrixMult(vector<vector<double> > v, vector<vector<double> > w){ //multiplies two matrix', if the dimensions are correct
	...
	erg.resize( v.size() , vector<double>( w[0].size() , 0 ) );                        // setting a matrix erg with 0 at every position and the correct dimensions
	unsigned int i;
	unsigned int j;
	unsigned int k;
...
vector<vector<double> > ExponentiationBySquaring(vector<vector<double> > v, double n){ // function that runs an exponantion in logarithmic time
